...
import { buildClaudeSettings, contextPreferencesFromRows } from '../scripts/qoder-gateway.mjs';
...
test('Claude setup preserves integrations and clears conflicting model credentials', () => {
  ...
  const { settings, contextWindow } = buildClaudeSettings(original, { baseUrl: 'http://127.0.0.1:18789', helper: 'node manager credentials', models: [{ id: 'qoder-anthropic/Kimi-K3', context_window: 1000000 }, { id: 'qoder-anthropic/Qwen3.8-Flash', context_window: 200000 }] });
...
test('desktop preferences retain only safe model keys and positive token windows', () => {
  assert.deepEqual(contextPreferencesFromRows([
    { model_key: 'gmodel', context_window: 1000000 },
    { model_key: 'bad key', context_window: 400000 },
    { model_key: 'qmodel', context_window: 0 },
    { model_key: 'other', context_window: '200000' },
  ]), { gmodel: 1000000, other: 200000 });
});
